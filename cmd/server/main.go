...
func main() {
	...
	sched := scheduler.NewWithServices(accSvc, proxySvc, poolSvc, rl, cfg.Scheduler)
	...
	imagesH := &gateway.ImagesHandler{
		...
		Runner:                imageRunner,
		DAO:                   imageDAO,
		RouteResolver:         poolSvc,
		DefaultPoolID:         cfg.Gateway.DefaultImagePoolID,
		DefaultFallbackPoolID: cfg.Gateway.DefaultFallbackPoolID,
	}
	gwH.Images = imagesH // chat/completions 识别到图像模型时转派

	adminModelH := modelpkg.NewAdminHandler(modelDAO, modelReg, nil)
	...
	importCore := account.NewUnifiedImportCore(
		accSvc,
		poolSvc,
		accRefresher,
		accQuota,
